// 
// 
// Microsoft HTTP Server API Sample Application
// Source:  https://docs.microsoft.com/en-us/windows/win32/http/http-server-sample-application
// 
// This is the sample application provided by Microsoft at the above link as at Jan 4, 2022
// It illustrates the various steps / sections needed to successfully implement a basic working
// HTTP server.
// 
// NOTES:
// + This sample application uses HTTP Server API v1.0
// + The recommendation is to use HTTP Server API v2.0 going forward as it supports process isolation and least privilege
// 
// 
    ...
    while (!bStop)
    {
        ...
        if (NO_ERROR == result)
        {
            ...
            case HttpVerbGET:
                ...
                // Slightly modified from original sample Microsoft code to allow for an "END" request
                if ( wcscmp( pRequest->CookedUrl.pAbsPath, L"/end") == 0 ) {

                    bStop = true;

                    wprintf(L"Received END API call, %ws \n", 
